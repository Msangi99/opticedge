    ...
    <div class="admin-prod-page admin-prod-page--narrow">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-8">
            <div>
                ...
                <h1 class="admin-prod-title">Add expense</h1>
                <p class="admin-prod-subtitle">Record spend against a payment channel.</p>
            </div>
        ...
        <div class="admin-clay-panel admin-prod-form-shell overflow-hidden">
            ...
            <form action="{{ route('admin.expenses.store') }}" method="POST" class="admin-prod-form-body space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="activity" id="activity" value="{{ old('activity') }}" required
                        placeholder="e.g. Office supplies, transport…" class="admin-prod-input">
                    @error('activity')
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount') }}" required min="0" step="0.01" class="admin-prod-input">
                    @error('amount')
                        <p class="text-red-600 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="payment_option_id" id="payment_option_id" required class="admin-prod-select">
                        ...
                        @foreach($paymentOptions as $option)
                            <option value="{{ $option->id }}" @selected(old('payment_option_id') == $option->id)>
                ...
                <div class="admin-prod-form-footer !mt-0 !pt-0 !border-0 !shadow-none">
                    ...
                    <button type="submit" class="admin-prod-btn-primary px-8">Add expense</button>
                </div>
            </form>
        </div>
    </div>
