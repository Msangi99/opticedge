    ...
    <div class="admin-prod-page admin-prod-page--narrow">
        ...
        <div class="admin-clay-panel admin-prod-form-shell overflow-hidden">
            <div class="admin-prod-form-head">
                ...
                <p class="admin-prod-form-hint">Name, email, phone, branch, password, role, and ability for sign-in.</p>
            </div>
            <form method="POST" action="{{ route('admin.agents.store') }}" class="admin-prod-form-body space-y-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" class="admin-prod-select w-full max-w-xl" required>
                        <option value="agent" @selected(old('role', 'agent') === 'agent')>User (Agent)</option>
                        <option value="subadmin" @selected(old('role') === 'subadmin')>Subadmin</option>
                    </select>
                    @error('role')
                        <p class="text-red-600 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                    @enderror
