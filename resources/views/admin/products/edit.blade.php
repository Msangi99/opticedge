<x-admin-layout>
    <div class="py-12 px-8 max-w-5xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Edit Stock</h1>
                <p class="mt-2 text-slate-500">Update inventory for {{ $product->name }}.</p>
            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    @foreach($product->images as $image)
                                        <div class="relative group">
                                            <img src="{{ Storage::url($image) }}" alt="Product Image"
                                                class="h-20 w-full object-cover rounded-md border border-slate-200">
                                            <!-- Logic to delete single image could be added here later -->
                                        </div>
                                    @endforeach
                                </div>
                                <p class="text-xs text-slate-400 mt-1">Note: Uploading new images adds to existing ones (max
                                    5 total). Delete functionality can be added.</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col h-full">
                        ...
                        <textarea name="description" id="description"
                            ...
                            placeholder="Enter device specifications, features, and condition details...">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>
                        @enderror
