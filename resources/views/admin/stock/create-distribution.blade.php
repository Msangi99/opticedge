            ...
            <form method="POST" action="{{ route('admin.stock.store-distribution') }}" class="admin-prod-form-body space-y-6" id="dist-form">
                ...
                <div>
                    ...
                    @error('dealer_id')
                        <p class="text-red-600 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                    @enderror
                    <p class="helper-text">Select from list or create new dealer in settings</p>
                </div>
                ...
                <div class="admin-prod-form-footer !mt-0 !pt-0 !border-0 !shadow-none">
                    ...
                    <button type="submit" class="admin-prod-btn-primary px-8" id="submit-btn" onclick="return validateDistForm()">Record sale</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function updatePaymentStatus() {
            ...
            if (total === 0) {
                ...
                paymentStatus.textContent = '⚠️ Overpaid by ' + formatCurrency(paid - total) + ' TZS';
        ...
        function validateSubmit() {
            ...
            if (quantity > 0 && price > 0 && total > 0 && paid >= 0 && dealerSelected && productSelected && paid <= total * 1.1) {
