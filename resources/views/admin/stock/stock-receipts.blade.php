<x-admin-layout>
    <div class="py-12 px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <a href="{{ route('admin.stock.stocks.show', $stock->id) }}" class="text-sm text-slate-500 hover:text-slate-700 flex items-center gap-1 mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                    Back to {{ $stock->name }}
                </a>
                <h1 class="text-2xl font-bold text-slate-900">Payment Receipts - {{ $stock->name }}</h1>
                <p class="mt-2 text-slate-600">View payment receipts for purchases related to this stock.</p>
            </div>
        ...
        @else
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-12 text-center">
                ...
                <p class="text-slate-600 mb-4">No payment receipts have been uploaded for purchases in this stock yet.</p>
                <a href="{{ route('admin.stock.create-purchase', ['from_stock' => $stock->id]) }}" class="inline-block bg-[#fa8900] text-white px-4 py-2 rounded-lg hover:bg-[#fa8900]/90 transition-colors font-medium">
